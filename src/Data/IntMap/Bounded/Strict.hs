...
module Data.IntMap.Bounded.Strict (
    -- * Map type
      IntMap, Key
...
alterF f k (IntMap m) = fmap IntMap (W.alterF f (i2w k) m)
...
traverseWithKey f = start
  where
    ...
    goR Tip = pure Tip
    goR (Bin min minV l r) = (\(!v') l' r' -> Bin min v' l' r') <$> f (w2i min) minV <*> goL l <*> goR r
